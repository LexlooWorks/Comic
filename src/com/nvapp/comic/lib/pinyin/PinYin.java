...
import com.nvapp.comic.lib.pinyin.HanziToPinyin3.Token;
...
public class PinYin {
	public static class PinYinElement {
		...
		public String toString() {
			StringBuilder part1 = new StringBuilder(
					"PinYinElement [pinyin=" + pinyin + ", firstChars=" + tokenFirstChars + "]");
	...
	public static void getPinYin(String input, PinYinElement pinyinElement) {
		...
		if (tokens != null && tokens.size() > 0) {
			for (Token token : tokens) {
				...
				if (Token.PINYIN == token.type) {
					...
					// 你xyz好 -> token.source = xyz, so we should separate every
					// original character here
					sb.append(token.source);
					for (int i = 0; i < token.source.length(); ++i) {
						String childString = token.source.substring(i, i + 1).toUpperCase(Locale.CHINESE);
		...
		String ret = sb.toString().toUpperCase(Locale.CHINESE);
		if (!ret.isEmpty()) {
